...
4. **Melhoria da arquitetura visando qualidade e manutenibilidade:** Com mentoria técnica, o sistema foi refatorado para respeitar o **Princípio de Inversão de Dependência**, removendo dependências diretas do construtor da `Carteira` na classe `Cliente`, e introduzindo um service para outras operações. Nesse um terceiro momento, com o aumento da quantidade de regras de consulta e relatórios (como filtros, ordenação, somatórios e rankings), foi criada a classe ClienteService, que passou a orquestrar operações mais complexas sobre listas de clientes, utilizando o ClienteRepository como fonte de dados. Esse service não substitui a lógica de domínio nas entidades, mas atua como coordenador de casos de uso mais elaborados (por exemplo, gerar ranking ou remover clientes com saldo zero), aproximando o projeto de um modelo de domínio híbrido, em que parte significativa da lógica permanece nas entidades (característica de Rich Domain Model) e outra parte é organizada em services e funções utilitárias.
